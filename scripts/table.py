...
class Table:
    ...
    def __makeTopDlg(self):
        ...
        self.__tree = ttk.Treeview(self.__dlgTop, columns=self.__hdu.columns.names, show='headings')
        ...
        if n < 1000:
            for i, row in enumerate(self.__hdu.data):
                self.__tree.insert('', tk.END, values=tuple(row))
        else:
            self.__tree.insert('', tk.END, values=tuple(self.__hdu.data[0]))
            self.__tree.insert('', tk.END, values=tuple(self.__hdu.data[1]))
            self.__tree.insert('', tk.END, values=tuple(self.__hdu.data[2]))
            self.__tree.insert('', tk.END, values=tuple(self.__hdu.data[n - 3]))
            self.__tree.insert('', tk.END, values=tuple(self.__hdu.data[n - 2]))
            self.__tree.insert('', tk.END, values=tuple(self.__hdu.data[n - 1]))
        self.__tree.pack()
